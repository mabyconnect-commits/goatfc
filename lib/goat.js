...
function gpTriggerRound(net, roundId) {
  const day = Math.floor(roundId / GP.MINUTES_PER_DAY);
  const { SESSION_SECRET } = cfg();
  const off = parseInt(crypto.createHmac("sha256", SESSION_SECRET || "dev").update(`gp-jackpot:${normNet(net)}:${day}`).digest("hex").slice(0, 8), 16) % GP.MINUTES_PER_DAY;
  return { day, triggerRound: day * GP.MINUTES_PER_DAY + off };
}
...
async function gpResolve(net) {
  ...
  for (let r = cur - 1; r >= cur - 5 && r >= 0; r--) { // look back a few rounds only
